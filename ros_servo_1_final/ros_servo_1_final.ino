...
void init_Pins() {//初始化对应的控制舵机引脚
  ...
  originalAngle[0] = 105;//设置初始舵机角度 小（放） 大 （收） 
  servoMIN[0] = 15;//对应舵机最小脚
  servoMAX[0] = 180;//对应舵机最大脚
  
  PIN[1] = 9;//板子上引脚9 /joint1 腕关节
  originalAngle[1] = 40;//设置初始舵机角度 90  大 - 小 
  servoMIN[1] = 40;//对应舵机最小脚
  servoMAX[1] = 135;//对应舵机最大脚
  
  PIN[2] = 8;//板子上引脚8 /joint2 臂关节
  originalAngle[2] = 80;//设置初始舵机角度  大  小
...
void messageCb0(const std_msgs::Int8& msg){
  ...
  if(msg.data==0){//初始位置
   ...
   else if(msg.data==5){//放开
    //臂关节~135
    rosServo[0].write(45);//爪子
    }
   else if(msg.data==6){//预备夹持 for fuck1
    rosServo[2].write(80);//臂关节
    rosServo[1].write(40);//腕关节
    rosServo[0].write(30);//爪子
    }
