...
void messageCb0(const std_msgs::Int8& msg){
  ...
  if(msg.data==0){//初始位置
    ...
    for(int i=152;i>=50;i--){//臂关节152-45
      rosServo[2].write(i);      
      delay(10);
      }
    ...
    rosServo[0].write(25);//爪子
    for(int i=50;i<=140;i++){//臂关节45-152
      rosServo[2].write(i);
      if(i<=90){
        rosServo[1].write(135+15-i);//腕关节90-45
        }
      delay(10);
      }
    }
    ...
    for(int i=152;i>=45;i--){//臂关节152-45
      rosServo[2].write(i);
      if(i<=145 && i>=65){//腕关节45-90
        rosServo[1].write(190-i);
        }
      delay(10);
      }
